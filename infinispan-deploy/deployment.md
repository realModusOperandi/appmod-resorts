# Deploying to OpenShift with Infinispan Session Persistence
...
*Reference: https://github.com/OpenLiberty/open-liberty-operator/tree/master/deploy/releases/0.5.1#open-liberty-operator-v051*
...
Install the Infinispan operator by following the instructions on operatorhub.io: https://operatorhub.io/operator/infinispan

See the reference link for manual installation instructions, but note that they are out of date.
...
When the server pod's status is Ready and the line `Infinispan Server 10.1.5.Final started in 21301ms` appears in the logs, the server is ready.
...
```
The Infinispan configuration will be added automatically at build time.
...
RUN configure.sh
```
 This is a multi-stage build. The first stage uses scripting in the Liberty image to fetch the Infinispan dependency jars from Maven Central.

In the second stage, the Dockerfile copies over the dependencies and places the app, `jvm.options`, and `server.xml` files in the appropriate place. Set the value of `INFINISPAN_SERVICE_NAME` to the name of the service resource created for your Infinispan cluster. This will be the name of the Infinispan resource if using the Infinispan or Data Grid operator.

Build this image, tag it, and push it. You may need to log in to your Docker registry (execute commands from the `docker` directory in the project.)

> If you are pushing to the internal Openshift registry, make sure you tag it with the same project you deployed the Infinispan to, so the application can be deployed to that project as well. If you want to deploy the application to a different project, you'll have to manually define some environment variables, see the Liberty container documentation: https://github.com/WASdev/ci.docker/blob/master/README.md#session-caching.
...
metadata:
  name: modresorts
spec:
  applicationImage: YOUR_IMAGE_HERE
  replicas: 2
  service:
    type: ClusterIP
    port: 9080
  ...
  volumes:
    - name: infinispan-secret-volume
      secret:
        secretName: modresorts-cache-generated-secret
  env:
    - name: WLP_LOGGING_CONSOLE_FORMAT
      value: "simple"
```

Update the `applicationImage` field with the name of the image you pushed, and the `secretName` field with the name of the secret containing the Infinispan user credentials. The operator will automatically generate this for you, or you can create it yourself.
